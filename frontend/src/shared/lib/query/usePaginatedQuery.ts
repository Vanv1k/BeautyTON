...
import { extractFirstPaginatedPage } from '../../lib/request/extractFirstPaginatedPage';
import type { SomePartial } from '../../lib/types/SomePartial';
...
> & {
  /**
   * Реакция на изменение полученных данных. Получает на вход последнюю полученную страницу пагинации
   */
  effect?: (params: PaginatedQueryEffectParams<T>) => void;

  /**
   * Реакция на изменение полученных данных в момент смены fetchStatus из 'fetching' в 'idle'. Получает на вход последнюю полученную страницу пагинации
   */
  fetchedEffect?: (params: PaginatedQueryEffectParams<T>) => void;
...
>({
  effect,
  ...queryInit
}: PaginatedQueryParams<T, ResponseT>): PaginatedQueryResult<T, ResponseT> => {
  ...
  React.useEffect(() => {
    ...
    if (lastPage && !lastPage.isError) {
      effect?.({
        lastPage: lastPage.data.paginationEntity.results,
      });
    }
  }, [query.data?.pages, effect]);
  ...
  return {
    ...
    total: firstPage?.paginationEntity.pagination.total ?? 0,
  };
};
